...
int main(int argc, char** argv) {
  ...
  while (zmq_config.context->handle() != nullptr) {
    ...
    if (sockets.receive_bytes(msg)) {
      ...
      auto error = target.get_positions() - joint_state.get_positions();
      auto torques = gain * error - damping * joint_state.get_velocities();
      auto command = state_representation::JointTorques("franka", joint_state.get_names(), torques);
